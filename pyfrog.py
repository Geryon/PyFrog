...
#    pygame.mixer.init()
    ...
    screen = pygame.display.set_mode((Global.Screen_width,
...
#                goals[i].occupied = 0
        ...
        ## Move everything
        for n in range(Global.Max_vehicles):
            game.vehicle[n].update(game)
        for n in range(Global.Max_logs):
            game.log[n].update(game)
        for n in range(Global.Max_turtles):
            game.turtle[n].update(game)
...
def configGameScreen():
    ...
    ## Configure goals
    for n in range(Global.Max_goals):
        pass

    ## Configure logs
    for n in range(Global.Max_logs):
        game.log.append(objects.Logs(n))

    ## Configure turtles
    for n in range(Global.Max_turtles):
        game.turtle.append(objects.Turtles(n))

    ## Configure vehicles
    for n in range(Global.Max_vehicles):
        game.vehicle.append(objects.Vehicles(n))
...
def drawGameScreen():
    ...
    screen.blit(gfx.background_image, (0, 0))
    for n in range(Global.Max_vehicles):
        game.vehicle[n].draw(screen, gfx, game)
    for n in range(Global.Max_logs):
        game.log[n].draw(screen, gfx)
    for n in range(Global.Max_turtles):
        game.turtle[n].draw(screen, gfx)
...
def ridingFrog():
    ...
    if frog.riding == Global.Left:
        frog.pos = (X - speed, Y)
    elif frog.riding == Global.Right:
        frog.pos = (X + speed, Y)
